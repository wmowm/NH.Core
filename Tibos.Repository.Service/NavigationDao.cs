...
{
	public class NavigationDao:INavigation
	{
        ...
        /// <returns></returns>
        public virtual IList<Navigation> GetList(RequestParams request)
        {
            using (ISession session = sessionFactory.OpenSession())
            {
                ICriteria crit = session.CreateCriteria(typeof(Navigation));
                IList<Navigation> list = ManagerPage.GetCrit<Navigation>(request,crit);
                return list;
            }
        }
        ...
        /// <returns></returns>
        public virtual IList<Navigation> GetList(Expression<Func<Navigation, bool>> expression, List<SortOrder<Navigation>> expressionOrder, Pagination pagination)
        {
            using (ISession session = sessionFactory.OpenSession())
            {
                var query = session.QueryOver<Navigation>().Where(expression);
                IList<Navigation> list = ManagerPage.GetQueryOver<Navigation>(query, expressionOrder, pagination);
                return list;
            }
        }
        ...
        public virtual int GetCount(RequestParams request)
        {
            ...
            {
                ICriteria crit = session.CreateCriteria(typeof(Navigation));
                int count = ManagerPage.GetCritCount<Navigation>(request, crit);
                return count;
            }
        }
